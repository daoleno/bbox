...
export async function fetchNFT({ search, setNFTs, fetchIndex, setFetchIndex }) {
  const contractAbi = require("../contracts/erc721/erc721abi.json");
  ...
  const contract = new Contract(search, contractAbi, provider);

  try {
    const totalSupply = await contract.totalSupply();
    for (let i = fetchIndex; i <= totalSupply; i++) {
      let tokenURI;
      try {
        ...
        //   error.message.includes("Token ID does not exist")
        // ) {
        continue;
        // }
        // throw error;
      ...
      const data = await (await fetch(toCorsHttp(tokenURI))).json();
      console.log(data);
...
export function parseIPFSURI(uri) {
  ...
  if (uri.startsWith(ipfsPrefix)) {
    return uri.substr(ipfsPrefix.length);
  } else {
    return uri;
  }
}
